...
export default function DashboardPage() {
  return (
    <section className="container-base py-10">
      <Helmet><title>Dashboard | Staynest</title></Helmet>
      <div className="mb-4">
        <BackButton />
      </div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-semibold">My Bookings</h1>
        <Link to="/host/add" className="rounded-xl bg-brand-primary px-4 py-2 text-white hover:bg-brand-dark">
          Add Listing
        </Link>
      </div>
      ...
      <article className="mb-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {mockBookings.map((booking) => {
              const listing = mockListings.find((item) => item.id === booking.listingId);
              return (
                <article key={booking.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white transition hover:-translate-y-1 hover:shadow-soft">
                  <img src={listing?.images?.[0]} alt={booking.listingTitle} className="h-44 w-full object-cover" loading="lazy" />
